...
const { MockWebSocket } = vi.hoisted(() => {
  class MockWebSocket {
    ...
    private _sendHandler: ((msg: string) => void) | null = null;
    ...
    setSendHandler(handler: (msg: string) => void): void {
      this._sendHandler = handler as ((msg: string) => void) | null;
    }
...
describe('CDPPage frames', () => {
  ...
  it('lists cross-origin frames from Page.getFrameTree', async () => {
    ...
    vi.spyOn(bridge, 'send')
      .mockResolvedValueOnce({}) // Page.enable (connect)
      .mockResolvedValueOnce({}) // stealth (connect)
      .mockResolvedValueOnce({ // Page.getFrameTree (frames)
        frameTree: {
...
describe('CDPPage evaluateInFrame', () => {
  ...
  it('evaluates JS in a specific frame', async () => {
    ...
    (page as unknown as { _frameContexts: Map<string, number> })._frameContexts.set('frame-1', 42);
