 ...
 */
export function isHumanModeEnabled(): boolean {
...
export interface HumanConfig {
  ...
  mouseSpeedMin: number;  // pixels per second
...
export function getHumanConfig(): HumanConfig {
  ...
  // Mouse overrides
  if (process.env.OPENCLI_HUMAN_MOUSE_SPEED_MIN) cfg.mouseSpeedMin = parseInt(process.env.OPENCLI_HUMAN_MOUSE_SPEED_MIN, 10);
  if (process.env.OPENCLI_HUMAN_MOUSE_SPEED_MAX) cfg.mouseSpeedMax = parseInt(process.env.OPENCLI_HUMAN_MOUSE_SPEED_MAX, 10);
  if (process.env.OPENCLI_HUMAN_MOUSE_JITTER) cfg.mouseJitterRange = parseFloat(process.env.OPENCLI_HUMAN_MOUSE_JITTER);
  if (process.env.OPENCLI_HUMAN_MOUSE_OVERSHOOT_PROB) cfg.mouseOvershootProbability = parseFloat(process.env.OPENCLI_HUMAN_MOUSE_OVERSHOOT_PROB);

  // Keyboard overrides
  if (process.env.OPENCLI_HUMAN_TYPO_PROB) cfg.keyTypoProbability = parseFloat(process.env.OPENCLI_HUMAN_TYPO_PROB);
  if (process.env.OPENCLI_HUMAN_KEY_DELAY_MIN) cfg.keyDelayMinMs = parseInt(process.env.OPENCLI_HUMAN_KEY_DELAY_MIN, 10);
  if (process.env.OPENCLI_HUMAN_KEY_DELAY_MAX) cfg.keyDelayMaxMs = parseInt(process.env.OPENCLI_HUMAN_KEY_DELAY_MAX, 10);

  // Scroll overrides
  if (process.env.OPENCLI_HUMAN_SCROLL_STEP_MIN) cfg.scrollStepMinPx = parseInt(process.env.OPENCLI_HUMAN_SCROLL_STEP_MIN, 10);
  if (process.env.OPENCLI_HUMAN_SCROLL_STEP_MAX) cfg.scrollStepMaxPx = parseInt(process.env.OPENCLI_HUMAN_SCROLL_STEP_MAX, 10);
  if (process.env.OPENCLI_HUMAN_SCROLL_PAUSE_MIN) cfg.scrollPauseMinMs = parseInt(process.env.OPENCLI_HUMAN_SCROLL_PAUSE_MIN, 10);
  if (process.env.OPENCLI_HUMAN_SCROLL_PAUSE_MAX) cfg.scrollPauseMaxMs = parseInt(process.env.OPENCLI_HUMAN_SCROLL_PAUSE_MAX, 10);
 ...
 */
export function generateOvershootCurve(
  targetX: number,
  targetY: number,
  startX: number,
  startY: number,
 ...
 */
export function getAdjacentKey(char: string): string | null {
