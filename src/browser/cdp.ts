...
export class CDPBridge implements IBrowserFactory {
  ...
  async connect(opts?: { timeout?: number; workspace?: string; cdpEndpoint?: string; initialUrl?: string }): Promise<IPage> {
    ...
    try {
      ...
      const version = await fetchJsonDirect(`${this._cdpEndpoint}/json/version`) as { webSocketDebuggerUrl?: string };
      ...
      const targets = await fetchJsonDirect(`${this._cdpEndpoint}/json`) as CDPTarget[];
      ...
      const allTargets = await fetchJsonDirect(`${this._cdpEndpoint}/json`) as CDPTarget[];
   ...
   */
  private async _connectBrowserTemp(wsUrl: string, timeoutMs: number): Promise<WebSocket> {
   ...
   */
  private _sendOnWs(ws: WebSocket, method: string, params: Record<string, unknown> = {}, timeoutMs: number = CDP_SEND_TIMEOUT): Promise<unknown> {
    return new Promise((resolve, reject) => {
   ...
   */
  private async _connectPage(wsUrl: string, timeoutMs: number): Promise<void> {
    return new Promise((resolve, reject) => {
      const ws = new WebSocket(wsUrl);
      const timeout = setTimeout(() => {
        ws.close();
   ...
   */
  private _setupPageMessageHandler(ws: WebSocket): void {
   ...
   */
  private async _cleanupOnFailure(): Promise<void> {
...
class CDPPage extends BasePage {
  ...
  protected _currentTargetId: string | undefined; // Current page identity for setActivePage/getActivePage
